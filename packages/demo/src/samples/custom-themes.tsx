...
export default function CustomTheme() {
    const [selected, setSelected] = useState(AVAILABLE_THEMES[0]);
    const theme = THEMES[selected];
    return (
        <>
            <h1>Custom Themes</h1>
            <Chart {...options} {...theme.chart}>
                <AreaSeries data={data} {...theme.series}/>
            </Chart>
            <Switcher
                list={AVAILABLE_THEMES}
                value={selected}
                onChange={(e) => setSelected(e.target.value as keyof typeof THEMES)}
            />
        </>
    )
}
...
const AVAILABLE_THEMES = Object.keys(THEMES) as (keyof typeof THEMES)[];
