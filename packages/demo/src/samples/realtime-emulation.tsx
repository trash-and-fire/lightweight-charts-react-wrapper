...
import {BusinessDay, CrosshairMode, ISeriesApi} from 'lightweight-charts';
    ...
    useEffect(() => {
        ...
        function mergeTickToBar(price: number) {
            if (currentBar.open === null) {
                ...
                currentBar.close = price;
                currentBar.high = Math.max(currentBar.high as number, price);
                currentBar.low = Math.min(currentBar.low as number, price);
            }
            series.current?.update(currentBar);
        }
        ...
        function nextBusinessDay(time: BusinessDay) {
            const d = new Date();
